...
# --------------------------------------------------------------------------
# V50B Stage A - SWQOS entry + V47F/V47G watchdog exit stack.
# --------------------------------------------------------------------------
# Idempotency: refuse to start if a V50B python proc is already running.
# (We don't check tmux session here because the launcher itself runs inside
# tmux when invoked from tmux new-session.)
if pgrep -f "pgg2_v50b_stagea_live.py" >/dev/null 2>&1; then
  echo "V50B-LAUNCH-ABORT pgg2_v50b_stagea_live.py already running" >&2
  pgrep -af "pgg2_v50b_stagea_live.py" >&2
  exit 12
fi
...
export PGG2_V50B_STAGEA_FEE_BUDGET_SOL="${PGG2_V50B_STAGEA_FEE_BUDGET_SOL:-0.00025}"
export PGG2_V50B_MAX_FAILED_SENDS="${PGG2_V50B_MAX_FAILED_SENDS:-2}"
export PGG2_V50B_MAX_OPEN="${PGG2_V50B_MAX_OPEN:-1}"
export PGG2_V50B_MAX_CLOSES="${PGG2_V50B_MAX_CLOSES:-1}"
export PGG2_V50B_MAX_SECONDS="${PGG2_V50B_MAX_SECONDS:-2100}"
...
# Trade sizing (V50B forces a single small ticket).
export PGG2_TRADE_SIZE_SOL="${PGG2_TRADE_SIZE_SOL:-0.005}"

# V48 live-smoke envelope -- the V50B runner wraps V48 with our SWQOS sender
...
export PGG2_V48_MAX_OPEN="${PGG2_V50B_MAX_OPEN:-1}"
export PGG2_V48_MAX_SECONDS="${PGG2_V50B_MAX_SECONDS:-2100}"
...
# V50B exit-stack overrides -- V48's clean-close wait is REPLACED.
...
# Run.
RUNID="${RUNID:-pgg2_v50b_stagea_$(date +%Y%m%d_%H%M%S)}"
mkdir -p logs data
echo "$RUNID" > current_pgg2_v50b_stagea_runid.txt

export PGG2_V48_OUT_MD="/root/piggy/V50B_STAGEA_V48_DECISIONS.md"
export PGG2_V48_OUT_JSONL="/root/piggy/data/v50b_stagea_decisions.jsonl"
