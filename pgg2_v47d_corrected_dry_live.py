"""V47D - Corrected dry-live (no-send simulation of full entry+exit lifecycle).

Forks pgg2_v47d_no_send_capture.py and adds full lifecycle accounting:
- target = 10 non-negative closes (not 10 entries: safe failed buys don't
  count toward the target since they consume only signature fee)
- max 35 min wallclock
- failed_buy_fee_budget = 0.00100 SOL across all safe buy fails combined
- Each candidate becomes an "entry":
    - simulate buy outcome via branch materialization vs adverse curve
      drift (we use the V47B adverse_branch_outcome + the observed
      post-buy snapshot pnl)
    - if SAFE_BUY_FAIL projected -> count as failed_buy (2x sig fee charged)
    - else: track until BANK (>= +0.0006) or LOSS (<= -0.0005) or hold timeout
- Stop on first negative close OR fee budget exceeded
...
for _pat in _FORBIDDEN_CALL_PATTERNS:
    if _re.search(_pat, _src):
        sys.stderr.write(
            f"V47D-DRYLIVE-ABORT forbidden_call_pattern={_pat}\n"
        )
        sys.exit(2)
...
async def amain() -> int:
    ...
    try:
        ...
        print(f"V47D-DRYLIVE-ABORT import:{type(exc).__name__}:{exc}")
        return 2
    ...
    log(
        f"V47D-DRYLIVE start sizes={SIZE_SWEEP_SOL} "
    ...
    def _maybe_evaluate(mint, ts_ms_now, slot, sol_in, signer):
        ...
        jsonl_fp.flush()
        log(
            f"V47D-DRYLIVE-ENTRY mint={_short(mint)} size={final_size:.4f} "
    ...
    try:
        while _now_ms() < deadline_ms:
                ...
                for p in new_points:
                    ...
                    for key, rec in list(pending_candidates.items()):
                        ...
                        if pnl_now >= BANK_TH:
                            ...
                            log(
                                f"V47D-DRYLIVE-CLOSE bank pnl={pnl_now:+.6f} "
                        ...
                        if pnl_now <= LOSS_TH:
                            ...
                            pending_candidates.pop(key, None)
                            log(
                                f"V47D-DRYLIVE-CLOSE clamp_loss "
            ...
            if non_neg_closes >= args.target_non_neg_closes:
                ...
                break
            if failed_buy_fees_total >= FEE_BUDGET:
                stop_reason = "fee_budget_exceeded"
                break
            if now_ts >= next_progress_ms:
                log(
                    f"V47D-DRYLIVE progress "
    ...
    overall_pass = bool(
        ...
        and negative_closes == 0
        and budget_respected
        and net_pnl_sol > 0.0
        and elapsed_s <= args.max_seconds + 1
    )

    md_path = Path(args.out_md)
    md_path.parent.mkdir(parents=True, exist_ok=True)
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# V47D Corrected Dry-Live Result\n\n")
        ...
        f.write("\n## Per-entry table\n\n")
        f.write(
            "| # | mint | size | orig_size | downsized | ub | tbs | "
        ...
        f.write("\n## PASS Criteria\n\n")
        f.write(
            f"- 10 non_negative_closes within <=35min: "
            f"{'YES' if non_neg_closes >= args.target_non_neg_closes and elapsed_s <= args.max_seconds + 1 else 'NO'}\n"
        )
        f.write(
            f"- 0 negative_closes: "
    ...
    log(f"V47D-DRYLIVE wrote {md_path}")
    log(
        f"V47D-DRYLIVE done elapsed_s={elapsed_s:.1f} "
