...
from pgg2_v53_risk_intelligence import (
    V53RiskChecker, evaluate_risk_veto, DEFAULT_RULES,
)  # V54 risk veto
...
def install_broker_patches() -> None:
    ...
    # V54: SolanaTracker risk checker (one shared instance)
    _v54_st_key = os.environ.get("SOLANATRACKER_DATA_API_KEY", "").strip()
    try:
        _v54_risk = V53RiskChecker(_v54_st_key) if _v54_st_key else None
    except Exception as _e:
        _v54_risk = None
        sys.stderr.write(f"V54-RISK-INIT-FAIL err={_e}\n")
    ...
    def _v50b_retarget_buy(
        ...
        # V54: SolanaTracker risk veto BEFORE the buy is signed/sent.
        if _v54_risk is not None:
            try:
                _v54_features = _v54_risk.fetch(str(mint_str))
                _v54_ok, _v54_blockers = evaluate_risk_veto(_v54_features)
                _summary = (
                    f"score={_v54_features.get('score')} "
                    f"bndP={_v54_features.get('bundlers_pct')} "
                    f"devP={_v54_features.get('dev_pct')} "
                    f"sniP={_v54_features.get('snipers_pct')} "
                    f"top10={_v54_features.get('top10_pct')} "
                    f"holders={_v54_features.get('holders')} "
                    f"dangers={_v54_features.get('danger_names')}"
                )
                _log(
                    f"PGG2-V54-RISK-GATE mint={str(mint_str)[:14]}.. "
                    f"pass={_v54_ok} blockers={_v54_blockers} {_summary}"
                )
                if not _v54_ok:
                    raise RuntimeError(
                        f"v54_risk_veto_blocked mint={mint_str} blockers={_v54_blockers}"
                    )
            except RuntimeError:
                raise
            except Exception as _e:
                _log(
                    f"PGG2-V54-RISK-ERR mint={str(mint_str)[:14]}.. err={_e}"
                )
                raise RuntimeError(
                    f"v54_risk_check_failed mint={mint_str} err={_e}"
                )
