"""V47H - Corrected dry-live with V47H pre-entry rug veto added on top of
V47G (UNCHANGED).

Forks pgg2_v47g_corrected_dry_live.py and adds:
- V47H sell-aware buffer (extends V47C signer-aware buffer; tracks sell
  breadth and short curve-delta history).
- V47H rug veto called as the LAST step of the entry chain, BEFORE
  position open. Candidates that pass V47G but fail V47H are recorded
  as shadow-only (do not consume a max_open slot or enter the watchdog).
- V47H veto telemetry (vetos fired counts, shadow-only sample).
- Verdict assertion: no sub-500ms rugs may be admitted.

V47G base architecture (entry chain + watchdog + clean-close + max_open=2
+ drain) is UNCHANGED.
...
for _pat in _FORBIDDEN_CALL_PATTERNS:
    if _re.search(_pat, _src):
        sys.stderr.write(
            f"V47H-DRYLIVE-ABORT forbidden_call_pattern={_pat}\n"
        )
        sys.exit(2)
...
async def amain() -> int:
    ...
    try:
        ...
        print(f"V47H-DRYLIVE-ABORT import:{type(exc).__name__}:{exc}")
        return 2
    ...
    log(
        f"V47H-DRYLIVE start sizes={SIZE_SWEEP_SOL} "
    ...
    for env_name in (
        ...
        if os.environ.get(env_name, "0") != "1":
            log(f"V47H-DRYLIVE WARNING: {env_name} env not 1")
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# V47H Corrected Dry-Live Result\n\n")
