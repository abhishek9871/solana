...
SNIPE_AMOUNT_SOL = float(os.environ.get("V40_CORE_AMOUNT_SOL", "0.012"))
CORE_AMOUNT_SOL = SNIPE_AMOUNT_SOL
SCOUT_AMOUNT_SOL = float(os.environ.get("V40_SCOUT_AMOUNT_SOL", "0.006"))
MICRO_SCOUT_AMOUNT_SOL = float(os.environ.get("V40_MICRO_SCOUT_AMOUNT_SOL", "0.001"))
SCALE_IN_AMOUNT_SOL = float(os.environ.get("V40_SCALE_IN_AMOUNT_SOL", "0.006"))
MAX_POSITION_AMOUNT_SOL = float(os.environ.get("V40_MAX_POSITION_AMOUNT_SOL", "0.024"))
FULL_SIZE_SCORE = int(os.environ.get("V40_FULL_SIZE_SCORE", "8"))
MAX_CONCURRENT_POSITIONS = int(os.environ.get("V40_MAX_CONCURRENT_POSITIONS", "6"))
...
# TP fractions are fractions of REMAINING. Current price must be at/above trigger.
TP_RUNNER_MODE = True
TP_LADDER = [
    (1.18, 0.50),   # sell half only if current price is actually +18%, not if old peak was +18%
    (1.60, 0.50),   # sell half of the remainder at real +60%
    (2.50, 0.50),   # leave a moonbag after real 2.5x
]
...
async def cobuy_snipe(client: Client, kp: Optional[Keypair], mint: str, smart_names: list):
    ...
    A single wallet buy is only traded when the curve tape shows real SOL impulse.
    Late smart buys are accepted only as scout scalps with current breakout.
    """
    global session_pnl_sol, consec_losses
    try:
        if session_pnl_sol <= -MAX_SESSION_LOSS_SOL or consec_losses >= MAX_CONSEC_LOSSES:
            return
...
async def manage_position(client: Client, kp: Optional[Keypair], pos: Position):
    ...
    pnl = pos.realized_sol - pos.entry_amount_sol
    session_pnl_sol += pnl
    # Count only true positive net after paper drag as win.
    if pnl > 0:
        session_wins += 1
        consec_losses = 0
    else:
