...
cd /root/piggy
RUNID="pgg2_quote_paperwin_$(date -u +%Y%m%d_%H%M%S)"
...
# Exact signal config from the winning pgg2_artgate paper run.
...
# Quote-shadow validation: no signing/sending, but positions are priced from
# live Raptor quotes and charged conservative round-trip overhead.
