"""
Fast ORCAUSDT play with aggressive trail.
$8 margin × 10x = $80 notional.
TP +2% (~+$1.52 win, hits in minutes).
SL -1.2% (-$1.04 loss).
Plus trailer that moves SL to entry the moment price is +0.5% favorable.
"""
import os
...
TP_PCT = Decimal("0.05")     # +5% TP — same as last night's winner
SL_PCT = Decimal("0.007")    # -0.7% SL — tighter, faster stop
...
print(f"  TP:     ${tp_px} (+2% = +$1.52 win)")
print(f"  SL:     ${sl_px} (-1.2% = -$1.04 loss)")
print(f"  Now starting trailer to lock breakeven at +0.5% favorable...")
print(f"  Run: py orca_fast_trailer.py")
