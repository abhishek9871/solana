"""V47G - Corrected dry-live with V47G size-edge floor + downsizer + size-
tiered hold caps + mid-hold dump abort, on top of V47E two-buyer guard,
V47E clean-close, and max_open=2 concurrency cap.

Forks pgg2_v47f_corrected_dry_live.py and adds:
- V47G open-position quote watchdog spawned at each position open.
- Watchdog polls bonding-curve PDA via RPC every 250ms regardless of
  curve-subscriber state, providing exit decisions independent of feed.
- Earliest-wins between watchdog and accountSubscribe exit paths.
- V47F entry path (size_edge_floor, downsizer, hold_caps, midhold_dump_abort)
  UNCHANGED. Only the position monitor is modified.
...
for _pat in _FORBIDDEN_CALL_PATTERNS:
    if _re.search(_pat, _src):
        sys.stderr.write(
            f"V47G-DRYLIVE-ABORT forbidden_call_pattern={_pat}\n"
        )
        sys.exit(2)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--out-jsonl",
        default="/root/piggy/data/v47g_drylive_decisions.jsonl",
    )
...
async def amain() -> int:
    ...
    try:
        ...
        print(f"V47G-DRYLIVE-ABORT import:{type(exc).__name__}:{exc}")
        return 2
    ...
    log(
        f"V47G-DRYLIVE start sizes={SIZE_SWEEP_SOL} "
    ...
    for env_name in (
        ...
        if os.environ.get(env_name, "0") != "1":
            log(f"V47G-DRYLIVE WARNING: {env_name} env not 1")
    ...
    def _maybe_evaluate(mint, ts_ms_now, slot, sol_in, signer):
        ...
        if not v47g_pass_floor:
            ...
            if new_size is None:
                ...
                log(
                    f"PGG2-V47G-DRYLIVE-BLOCK mint={_short(mint)} "
                ...
                final_g = final_res["guard"]
                log(
                    f"PGG2-V47G-DRYLIVE-DOWNSIZED mint={_short(mint)} "
                    f"orig_size={v47g_orig_for_downsize:.4f} "
        ...
        try:
            ...
            log(
                f"V47G-DRYLIVE-WATCHDOG-SPAWN-ERROR mint={_short(mint)} "
                f"err={type(_wd_exc).__name__}:{_wd_exc}"
            )
        jsonl_fp.write(json.dumps(rec) + "\n")
        jsonl_fp.flush()
        log(
            f"V47G-DRYLIVE-ENTRY mint={_short(mint)} size={final_size:.4f} "
    ...
    try:
        while _now_ms() < deadline_ms:
                ...
                for p in new_points:
                    ...
                    for key, rec in list(pending_candidates.items()):
                        ...
                        if ab:
                            ...
                            open_positions.pop(key, None)
                            log(
                                f"V47G-DRYLIVE-CLOSE midhold_abort={ab_act} "
                        ...
                        if pnl_now >= BANK_TH:
                            ...
                            open_positions.pop(key, None)
                            log(
                                f"V47G-DRYLIVE-CLOSE bank pnl={pnl_now:+.6f} "
                                f"lag={lag}ms mint={_short(mint)} "
                        ...
                        if pnl_now <= LOSS_TH:
                            ...
                            open_positions.pop(key, None)
                            log(
                                f"V47G-DRYLIVE-CLOSE clamp_loss "
            ...
            if stop_trigger is not None:
                ...
                log(
                    f"V47G-DRYLIVE stop_trigger={stop_trigger} "
                    f"open_now={len(open_positions)} "
                        ...
                        for p in new_pts:
                            ...
                            for key_d, rec_d in list(
                                ...
                                if ab_d:
                                    ...
                                    log(
                                        f"V47G-DRYLIVE-DRAIN-CLOSE midhold_abort={ab_act_d} "
                                ...
                                if pnl_d >= BANK_TH:
                                    ...
                                    open_positions.pop(key_d, None)
                                    log(
                                        f"V47G-DRYLIVE-DRAIN-CLOSE bank "
                                ...
                                if pnl_d <= LOSS_TH:
                                    ...
                                    open_positions.pop(key_d, None)
                                    log(
                                        f"V47G-DRYLIVE-DRAIN-CLOSE clamp_loss "
                                        f"pnl={pnl_d:+.6f} lag={lag_d}ms "
                                        f"mint={_short(mint_drain)} "
                                        f"open_now={len(open_positions)}"
                                    )
                                ...
                                if lag_d >= hold_d:
                                    ...
                                    open_positions.pop(key_d, None)
                                    log(
                                        f"V47G-DRYLIVE-DRAIN-CLOSE {label_d} "
                                        f"pnl={pnl_d:+.6f} lag={lag_d}ms "
                                        f"mint={_short(mint_drain)} "
                                        f"open_now={len(open_positions)}"
                                    )
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# V47G Corrected Dry-Live Result\n\n")
    ...
    log(f"V47G-DRYLIVE wrote {md_path}")
    log(
        f"V47G-DRYLIVE done elapsed_s={elapsed_s:.1f} "
        f"non_neg={non_neg_closes} neg={negative_closes} "
