"""V47H - No-Send capture with V47H PRE-ENTRY RUG VETO added on top of
V47G (UNCHANGED).

Forks pgg2_v47g_no_send_capture.py and adds:
  - V47H sell-aware buffer (extends V47C signer-aware buffer; tracks
    unique sellers, sell breadth and short curve-delta history).
  - V47H rug veto called as the LAST step of the entry chain (after all
    V47G filters), before candidate record build / pending_candidates
    bookkeeping.
  - V47H veto telemetry + shadow-only collection for blocked entries.

V47G base architecture is UNCHANGED. All V47B/C/D/E/F/G modules invoked
identically. The veto only adds a final causal block; candidates that
pass V47G but fail V47H are recorded as shadow-only (do not count toward
target_pass).
...
for _pat in _FORBIDDEN_CALL_PATTERNS:
    if _re.search(_pat, _src):
        sys.stderr.write(
            f"V47H-NO-SEND-ABORT forbidden_call_pattern={_pat}\n"
        )
        sys.exit(2)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--out-jsonl",
        default="/root/piggy/data/v47h_no_send_decisions.jsonl",
    )
...
async def amain() -> int:
    ...
    try:
        ...
        print(f"V47H-NO-SEND-ABORT import:{type(exc).__name__}:{exc}")
        return 2
    ...
    log(
        f"V47H-NO-SEND start sizes={SIZE_SWEEP_SOL} "
    ...
    for env_name in (
        ...
        if os.environ.get(env_name, "0") != "1":
            log(f"V47H-NO-SEND WARNING: {env_name} env not 1")
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"- OVERALL_VERDICT: {'PASS' if v47h_overall else 'FAIL'}\n\n")
