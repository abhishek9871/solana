"""V47D - No-Send capture with boundary-loss guard + downsize-before-block
+ replacement router.

Forks pgg2_v47c_no_send_capture.py and adds:
  - V47D boundary guard (Phase 2 of V47D plan) after V47C size cap
  - V47D downsizer (Phase 3): if guard fails at original size, try smaller
    admissible sizes before fully blocking
  - V47D replacement router (Phase 4): keep scanning past blocked candidates
    until target_pass accepted or timeout reached

Order of evaluation per raw Pump BUY event:
  1. Compute buyer_stats from signer-aware buffer
  2. Multi-buyer gate (V47C) BEFORE size sweep
  3. Run V47B guarded-branch sweep over all sizes
  4. Among selectable sizes, apply V47C size cap
  5. Pick smallest admissible from V47C cap
  6. Apply V47D boundary guard at that size
  7. If V47D blocks: invoke downsizer to find smaller admissible size
  8. If downsizer also blocks: emit REPLACEMENT-SCAN log; continue listening
  9. If accepted: emit candidate record; continue if more candidates needed
...
for _pat in _FORBIDDEN_CALL_PATTERNS:
    if _re.search(_pat, _src):
        sys.stderr.write(
            f"V47D-NO-SEND-ABORT forbidden_call_pattern={_pat}\n"
        )
        sys.exit(2)
...
async def amain() -> int:
    ...
    try:
        ...
        print(f"V47D-NO-SEND-ABORT import:{type(exc).__name__}:{exc}")
        return 2
    ...
    log(
        f"V47D-NO-SEND start sizes={SIZE_SWEEP_SOL} "
    ...
    for env_name in (
        ...
        "V47D_NO_SEND",
        "PGG2_V47D_BOUNDARY_LOSS_GUARD",
        "PGG2_V47D_DOWNSIZE_BEFORE_BLOCK",
        "PGG2_V47D_REPLACEMENT_ROUTER",
    ):
        if os.environ.get(env_name, "0") != "1":
            log(f"V47D-NO-SEND WARNING: {env_name} env not 1")
    ...
    log(
        f"V47D-NO-SEND fee_bps={fee_bps} creator_fee_bps={creator_fee_bps}"
    )
    ...
    async def _shred_listener():
        ...
        try:
            ...
            log(f"V47D-NO-SEND ws_import_err={exc}")
            return
        url = os.environ.get("SOLANATRACKER_RPC_WS", "")
        if not url:
            log("V47D-NO-SEND no_ws_url")
            return
        backoff = 2.0
        while not shred_stop.is_set():
            try:
                async with websockets.connect(
                    ...
                    log("V47D-NO-SEND shred_subscribed")
                    async for raw in ws:
                ...
                log(f"V47D-NO-SEND shred_reconnect "
    ...
    def _maybe_evaluate(
        ...
        if bg_pass:
            ...
            if d_size is None:
                ...
                log(
                    f"PGG2-V47D-REPLACEMENT-SCAN ts={ts_ms_now} "
                ...
                if downsized_bool:
                    log(
                        f"PGG2-V47D-REPLACEMENT-SELECTED mint={_short(mint)} "
        ...
        jsonl_fp.flush()
        log(
            f"V47D-CANDIDATE mint={_short(mint)} "
                ...
                for p in new_points:
                    ...
                    for key, rec in list(pending_candidates.items()):
                        ...
                        if pnl_now >= BANK_TH:
                            ...
                            log(
                                f"V47D-OBSERVED bank pnl={pnl_now:+.6f} "
                        ...
                        if pnl_now <= LOSS_TH:
                            ...
                            pending_candidates.pop(key, None)
                            log(
                                f"V47D-OBSERVED clamp_loss pnl={pnl_now:+.6f} "
                                f"lag={lag}ms mint={_short(mint)} "
                                f"size={size_sol:.4f}"
                            )
                            continue
                        ...
                        if lag >= hold_ms:
                            ...
                            pending_candidates.pop(key, None)
                            log(
                                f"V47D-OBSERVED {label_kind} pnl={pnl_now:+.6f} "
                                f"lag={lag}ms mint={_short(mint)}"
                            )

            if now_ts >= next_progress_ms:
                log(
                    f"V47D-NO-SEND progress elapsed_s="
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# V47D - Boundary-Guarded No-Send Report\n\n")
        ...
        f.write("- rules_path: (in-code; see pgg2_v47d_boundary_guard.py "
                "+ pgg2_v47d_downsizer.py)\n\n")
        ...
        if len(candidates) == 0:
            f.write(
                f"- V47D yielded 0 candidates in {elapsed_s:.0f}s. "
                f"boundary_guard_total={boundary_guard_total}, "
                f"downsize_attempts={downsize_attempt_count}, "
                f"replacement_scans={replacement_scan_count}. "
                f"Top mb-gate blockers: "
                f"{dict(mb_gate_blocker_counts.most_common(5))}. "
                f"Top boundary blockers: "
                f"{dict(boundary_guard_blocker_counts.most_common(5))}. "
            )
        elif clamp_count + expired_loss_count > 0:
            f.write(
                f"- V47D produced {len(candidates)} candidate(s) but "
                f"{clamp_count + expired_loss_count} resulted in observed "
                f"negative outcomes. Boundary guard / downsizer did not "
                f"catch the failure pattern.\n"
            )
        elif meets_target and zero_neg:
            f.write(
                f"- V47D PASSED: {len(candidates)} candidates, 0 negative "
                f"outcomes, all sanity checks satisfied. Boundary guard "
                f"+ downsizer + replacement router operational.\n"
            )
        else:
            f.write(
                f"- V47D produced {len(candidates)} candidate(s) without "
                f"negatives but did not reach target ({args.target_pass}). "
                f"Replacement_scans={replacement_scan_count} indicates "
                f"how many candidates the boundary guard blocked.\n"
            )

    log(f"V47D-NO-SEND wrote {md_path}")
    log(
        f"V47D-NO-SEND done elapsed_s={elapsed_s:.1f} "
        f"candidates={len(candidates)} "
