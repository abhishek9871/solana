...
# --------------------------------------------------------------------------
# V54 Stage A - SWQOS entry + V47F/V47G watchdog exit stack.
# --------------------------------------------------------------------------
# Idempotency: refuse to start if a V54 python proc is already running.
# (We don't check tmux session here because the launcher itself runs inside
# tmux when invoked from tmux new-session.)
if pgrep -f "pgg2_v54_stagea_live.py" >/dev/null 2>&1; then
  echo "V54-LAUNCH-ABORT pgg2_v54_stagea_live.py already running" >&2
  pgrep -af "pgg2_v54_stagea_live.py" >&2
  exit 12
fi

# V54-specific knobs.
export PGG2_V54_LIVE=1
export PGG2_V54_SWQOS_TIP_SOL="${PGG2_V54_SWQOS_TIP_SOL:-0.000005}"
export PGG2_V54_MAX_TIP_SOL="${PGG2_V54_MAX_TIP_SOL:-0.000005}"
export PGG2_V54_STAGEA_FEE_BUDGET_SOL="${PGG2_V54_STAGEA_FEE_BUDGET_SOL:-0.00025}"
export PGG2_V54_MAX_FAILED_SENDS="${PGG2_V54_MAX_FAILED_SENDS:-2}"
export PGG2_V54_MAX_OPEN="${PGG2_V54_MAX_OPEN:-1}"
export PGG2_V54_MAX_CLOSES="${PGG2_V54_MAX_CLOSES:-1}"
export PGG2_V54_MAX_SECONDS="${PGG2_V54_MAX_SECONDS:-2100}"
export PGG2_V54_MAX_WALLET_DRAWDOWN_SOL="${PGG2_V54_MAX_WALLET_DRAWDOWN_SOL:-0.0030}"
export PGG2_V54_PRIORITY_FEE_MICROLAMPORTS="${PGG2_V54_PRIORITY_FEE_MICROLAMPORTS:-100000}"
export PGG2_V54_CU_LIMIT="${PGG2_V54_CU_LIMIT:-200000}"
export PGG2_V54_MAX_HOLD_MS="${PGG2_V54_MAX_HOLD_MS:-1500}"
# V47G watchdog knobs.
...
# Trade sizing (V54 forces a single small ticket).
export PGG2_TRADE_SIZE_SOL="${PGG2_TRADE_SIZE_SOL:-0.005}"

# V48 live-smoke envelope -- the V54 runner wraps V48 with our SWQOS sender
# AND overlays the V47G watchdog.
...
# Mirror direct_pump's priority-fee env.
export PGG2_DIRECT_PRIORITY_FEE_SOL=0.000020
export PGG2_DIRECT_COMPUTE_UNIT_LIMIT="${PGG2_V54_CU_LIMIT}"
export PGG2_DIRECT_COMPUTE_UNIT_PRICE_MICROLAMPORTS="${PGG2_V54_PRIORITY_FEE_MICROLAMPORTS}"
...
export PGG2_V48_TARGET_CLOSED_NONNEG="${PGG2_V54_MAX_CLOSES:-1}"
export PGG2_V48_MAX_OPEN="${PGG2_V54_MAX_OPEN:-1}"
export PGG2_V48_MAX_SECONDS="${PGG2_V54_MAX_SECONDS:-2100}"
...
# V54 exit-stack overrides -- V48's clean-close wait is REPLACED.
# These env vars make V48 aggressive: fire sell IMMEDIATELY on the first
# sell-loop iteration with a profit-preserving min_sol_out.
export PGG2_V48_LIVE_MAX_POSITION_MS="${PGG2_V54_MAX_HOLD_MS:-1500}"
export PGG2_V48_LIVE_SELL_MIN_PROFIT_SOL="${PGG2_V48_LIVE_SELL_MIN_PROFIT_SOL:--0.0003}"
# Effectively disable the pre-send PnL gate so V48 always attempts.
export PGG2_V48_LIVE_SELL_SEND_MIN_PNL_SOL="${PGG2_V48_LIVE_SELL_SEND_MIN_PNL_SOL:--0.500}"
# Buffer between protected min_sol_out and quote_out.
export PGG2_V48_LIVE_SELL_MIN_OUT_BUFFER_SOL="${PGG2_V48_LIVE_SELL_MIN_OUT_BUFFER_SOL:-0.00005}"
# Bound the emergency_close so it still preserves at least breakeven minus
# the accepted slip (size + 2*fee - 0.0003 ~= 0.0046 SOL on 0.005 trade).
...
# Run.
RUNID="${RUNID:-pgg2_v54_stagea_$(date +%Y%m%d_%H%M%S)}"
mkdir -p logs data
echo "$RUNID" > current_pgg2_v54_stagea_runid.txt

export PGG2_V48_OUT_MD="/root/piggy/V54_STAGEA_V48_DECISIONS.md"
export PGG2_V48_OUT_JSONL="/root/piggy/data/v54_stagea_decisions.jsonl"
...
exec "$PYTHON_BIN" -u pgg2_v54_stagea_live.py 2>&1 | tee -a "logs/${RUNID}.log"
