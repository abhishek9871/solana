"""V47F - Corrected dry-live with V47F size-edge floor + downsizer + size-
tiered hold caps + mid-hold dump abort, on top of V47E two-buyer guard,
V47E clean-close, and max_open=2 concurrency cap.

Forks pgg2_v47e_corrected_dry_live.py and adds:
- V47F size-edge floor (Phase 2) AFTER V47D boundary guard + downsize
- V47F large-size downsizer (Phase 3) on floor failure for size >= 0.030
- V47F size-tiered hold caps (Phase 4) replacing V47E's MAX_HOLD/EXTEND
- V47F mid-hold dump abort (Phase 5) called every position-management cycle
...
for _pat in _FORBIDDEN_CALL_PATTERNS:
    if _re.search(_pat, _src):
        sys.stderr.write(
            f"V47F-DRYLIVE-ABORT forbidden_call_pattern={_pat}\n"
        )
        sys.exit(2)
...
async def amain() -> int:
    ...
    try:
        ...
        print(f"V47F-DRYLIVE-ABORT import:{type(exc).__name__}:{exc}")
        return 2
    ...
    log(
        f"V47F-DRYLIVE start sizes={SIZE_SWEEP_SOL} "
    ...
    for env_name in (
        ...
        if os.environ.get(env_name, "0") != "1":
            log(f"V47F-DRYLIVE WARNING: {env_name} env not 1")
    ...
    v47f_midhold_abort_details: List[Dict[str, Any]] = []
    v47f_extend_skipped_large_size = 0
    v47f_hold_cap_forced_exits = 0
    ...
    def _maybe_evaluate(mint, ts_ms_now, slot, sol_in, signer):
        ...
        if len(open_positions) >= max_open:
            max_open_skips += 1
            log(
                f"PGG2-V47F-CONCURRENCY-CAP open_now={len(open_positions)} "
        ...
        # --- V47F Phase 2 + Phase 3: size-edge floor + large-size downsize.
        v47f_floor_total += 1
        cand_exp_pnl = float(final_r1["expected_pnl"])
        v47f_pass_floor, v47f_reason = evaluate_size_edge_floor(
            float(final_size), cand_exp_pnl,
            logger=log, mint_for_log=mint,
        )
        v47f_orig_for_downsize = float(final_size)
        if not v47f_pass_floor:
            ...
            if new_size is None:
                ...
                log(
                    f"PGG2-V47F-DRYLIVE-BLOCK mint={_short(mint)} "
                ...
                final_g = final_res["guard"]
                log(
                    f"PGG2-V47F-DRYLIVE-DOWNSIZED mint={_short(mint)} "
                    f"orig_size={v47f_orig_for_downsize:.4f} "
        ...
        v47f_caps_entry = get_hold_caps(float(final_size))
        rec = {
            "type": "v47f_drylive_entry",
            ...
            "v47f_max_hold_ms": int(v47f_caps_entry["max_hold_ms"]),
            "v47f_max_extend_ms": int(v47f_caps_entry["max_extend_ms"]),
            "v47f_extend_default_allowed": bool(v47f_caps_entry["extend_allowed"]),
            "v47f_extended_count": 0,
            "v47f_abort_triggered": False,
            "v47f_abort_reason": None,
            "close_kind": None,
            "close_pnl": None,
            "close_lag_ms": None,
            "is_failed_buy": False,
        ...
        jsonl_fp.flush()
        log(
            f"V47F-DRYLIVE-ENTRY mint={_short(mint)} size={final_size:.4f} "
    ...
    try:
        while _now_ms() < deadline_ms:
            ...
            for mint in list(hot_mint_last_seen.keys())[: args.max_hot_mints]:
                ...
                for p in new_points:
                    ...
                    for key, rec in list(pending_candidates.items()):
                        ...
                        prev_peak_q = float(rec.get("v47f_peak_quote", 0.0))
                        prev_peak_v = float(rec.get("v47f_peak_vsol", 0.0))
                        prev_peak_p = float(rec.get("v47f_peak_pnl", -1e18))
                        prev_last_q = float(rec.get("v47f_last_quote", 0.0))
                        prev_neg_g = int(rec.get("v47f_neg_grad_count", 0))
                        prev_above = bool(
                        ...
                        rec["v47f_peak_quote"] = new_peak_q
                        rec["v47f_peak_vsol"] = new_peak_v
                        rec["v47f_peak_pnl"] = new_peak_p
                        rec["v47f_last_quote"] = cur_q
                        rec["v47f_neg_grad_count"] = new_neg_g
                        rec["v47f_had_been_above_bank_or_scratch"] = new_above
                        try:
                        ...
                        if ab:
                            ...
                            v47f_midhold_abort_actions[str(ab_act)] += 1
                            v47f_midhold_abort_details.append({
                            ...
                            rec["v47f_abort_triggered"] = True
                            rec["v47f_abort_reason"] = ab_reason
                            ...
                            jsonl_fp.write(json.dumps({
                                ...
                                **{k: rec[k] for k in (
                                ...
                                "ts_ms": int(p.ts_ms),
                                "v47f_abort_triggered": True,
                                "v47f_abort_action": str(ab_act),
                                "v47f_abort_reason": str(ab_reason),
                            }) + "\n")
                            jsonl_fp.flush()
                            pending_candidates.pop(key, None)
                            open_positions.pop(key, None)
                            log(
                                f"V47F-DRYLIVE-CLOSE midhold_abort={ab_act} "
                        ...
                        if pnl_now >= BANK_TH:
                            ...
                            open_positions.pop(key, None)
                            log(
                                f"V47F-DRYLIVE-CLOSE bank pnl={pnl_now:+.6f} "
                                f"lag={lag}ms mint={_short(mint)} "
                        ...
                        if pnl_now <= LOSS_TH:
                            ...
                            open_positions.pop(key, None)
                            log(
                                f"V47F-DRYLIVE-CLOSE clamp_loss "
                            ...
                            stop_reason = "negative_close"
                            break
                        v47f_caps_rt = get_hold_caps(float(size_sol))
                        hold = int(v47f_caps_rt["max_hold_ms"])
                        rt_max_ext = int(v47f_caps_rt["max_extend_ms"])
                        rt_def_allowed = bool(v47f_caps_rt["extend_allowed"])
                        if float(size_sol) > 0.020 + 1e-9:
                            ...
                            extend_ok, extend_reason = check_extend_allowed(
                                float(size_sol),
                                {
                                    ...
                                    "no_negative_curve_update_last_250ms": (
                                        int(rec.get("v47f_neg_grad_count", 0)) == 0
                                    ),
                                },
                                logger=log, mint_for_log=mint,
                            )
                            if extend_ok:
                                ...
                                rec["v47f_extended_count"] = (
                                    int(rec.get("v47f_extended_count", 0)) + 1
                                )
                            else:
                                v47f_extend_skipped_large_size += 1
            ...
            if stop_trigger is not None:
                ...
                log(
                    f"V47F-DRYLIVE stop_trigger={stop_trigger} "
                    f"open_now={len(open_positions)} "
                    ...
                    for mint_drain in list(hot_mint_last_seen.keys())[
                        ...
                        for p in new_pts:
                            ...
                            for key_d, rec_d in list(
                                ...
                                # V47F midhold dump abort (drain).
                                prev_peak_qd = float(rec_d.get("v47f_peak_quote", 0.0))
                                prev_peak_vd = float(rec_d.get("v47f_peak_vsol", 0.0))
                                prev_peak_pd = float(rec_d.get("v47f_peak_pnl", -1e18))
                                prev_last_qd = float(rec_d.get("v47f_last_quote", 0.0))
                                prev_neg_gd = int(rec_d.get("v47f_neg_grad_count", 0))
                                prev_above_d = bool(rec_d.get("v47f_had_been_above_bank_or_scratch", False))
                                ...
                                rec_d["v47f_peak_quote"] = new_peak_qd
                                rec_d["v47f_peak_vsol"] = new_peak_vd
                                rec_d["v47f_peak_pnl"] = new_peak_pd
                                rec_d["v47f_last_quote"] = cur_qd
                                rec_d["v47f_neg_grad_count"] = new_neg_gd
                                rec_d["v47f_had_been_above_bank_or_scratch"] = new_above_d
                                try:
                                ...
                                if ab_d:
                                    v47f_midhold_abort_count += 1
                                    v47f_midhold_abort_actions[str(ab_act_d)] += 1
                                    ...
                                    rec_d["v47f_abort_triggered"] = True
                                    rec_d["v47f_abort_reason"] = ab_reason_d
                                    ...
                                    jsonl_fp.write(json.dumps({
                                        ...
                                        **{k: rec_d[k] for k in (
                                        ...
                                        "v47f_abort_triggered": True,
                                        "v47f_abort_action": str(ab_act_d),
                                        "v47f_abort_reason": str(ab_reason_d),
                                    }) + "\n")
                                    jsonl_fp.flush()
                                    pending_candidates.pop(key_d, None)
                                    open_positions.pop(key_d, None)
                                    log(
                                        f"V47F-DRYLIVE-DRAIN-CLOSE midhold_abort={ab_act_d} "
                                ...
                                if pnl_d >= BANK_TH:
                                    ...
                                    open_positions.pop(key_d, None)
                                    log(
                                        f"V47F-DRYLIVE-DRAIN-CLOSE bank "
                                ...
                                if pnl_d <= LOSS_TH:
                                    ...
                                    open_positions.pop(key_d, None)
                                    log(
                                        f"V47F-DRYLIVE-DRAIN-CLOSE clamp_loss "
                                        f"pnl={pnl_d:+.6f} lag={lag_d}ms "
                                        f"mint={_short(mint_drain)} "
                                        f"open_now={len(open_positions)}"
                                    )
                                    continue
                                v47f_caps_d = get_hold_caps(float(size_d))
                                hold_d = int(v47f_caps_d["max_hold_ms"])
                                rt_max_ext_d = int(v47f_caps_d["max_extend_ms"])
                                rt_def_allowed_d = bool(v47f_caps_d["extend_allowed"])
                                if float(size_d) > 0.020 + 1e-9:
                                    extend_ok_d, _ = check_extend_allowed(
                                        float(size_d),
                                        {
                                            ...
                                            "no_negative_curve_update_last_250ms": (
                                                int(rec_d.get("v47f_neg_grad_count", 0)) == 0
                                            ),
                                        },
                                    )
                                    if extend_ok_d:
                                ...
                                if lag_d >= hold_d:
                                    ...
                                    open_positions.pop(key_d, None)
                                    log(
                                        f"V47F-DRYLIVE-DRAIN-CLOSE {label_d} "
                                        f"pnl={pnl_d:+.6f} lag={lag_d}ms "
                                        f"mint={_short(mint_drain)} "
                                        f"open_now={len(open_positions)}"
                                    )
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# V47F Corrected Dry-Live Result\n\n")
        ...
        f.write("## V47F size-edge floor stats\n\n")
        ...
        f.write("\n## V47F downsize stats\n\n")
        f.write(f"- attempts: {v47f_downsize_attempts}\n")
        f.write(f"- success: {v47f_downsize_success}\n")
        f.write(f"- fail: {v47f_downsize_fail}\n")

        f.write("\n## V47F hold caps applied\n\n")
        f.write(f"- hold_cap_forced_exits: {v47f_hold_cap_forced_exits}\n")
        f.write(
            f"- extend_skipped_large_size (size>0.020): "
            f"{v47f_extend_skipped_large_size}\n"
        )
        ...
        if v47f_midhold_abort_actions:
            ...
            f.write("  - (none)\n")
        if v47f_midhold_abort_details:
            f.write("- per-mint detail:\n")
            for d in v47f_midhold_abort_details[:30]:
                f.write(
        ...
        for i, r in enumerate(candidates, 1):
            ...
            f.write(
                ...
                f"{int(r.get('v47f_max_hold_ms', 0))} | "
                f"{int(r.get('v47f_extended_count', 0))} | "
                f"{int(bool(r.get('v47f_abort_triggered', False)))} | "
    ...
    log(f"V47F-DRYLIVE wrote {md_path}")
    log(
        f"V47F-DRYLIVE done elapsed_s={elapsed_s:.1f} "
        f"non_neg={non_neg_closes} neg={negative_closes} "
