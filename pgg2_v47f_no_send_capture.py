"""V47F - No-Send capture with V47F size-edge floor + large-size downsizer
on top of V47E two-buyer + V47D boundary + downsize + clean-close.

Forks pgg2_v47e_no_send_capture.py and adds (after V47D Phase 3 downsize):
  - V47F Phase 2: size-tiered exp_pnl floor (Phase 2 of V47F).
  - V47F Phase 3: large-size downsize-before-block.
  - V47F Phase 5: mid-hold dump abort simulation against future snaps.

For no-send: hold caps and dump abort act on the same future-snap trajectory
that V47E uses for labeling. If V47F mid-hold dump abort triggers BEFORE the
...
for _pat in _FORBIDDEN_CALL_PATTERNS:
    if _re.search(_pat, _src):
        sys.stderr.write(
            f"V47F-NO-SEND-ABORT forbidden_call_pattern={_pat}\n"
        )
        sys.exit(2)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--out-jsonl",
        default="/root/piggy/data/v47f_no_send_decisions.jsonl",
    )
...
async def amain() -> int:
    ...
    try:
        ...
        print(f"V47F-NO-SEND-ABORT import:{type(exc).__name__}:{exc}")
        return 2
    ...
    log(
        f"V47F-NO-SEND start sizes={SIZE_SWEEP_SOL} "
    ...
    for env_name in (
        ...
        "V47F_NO_SEND",
        "PGG2_V47F_SIZE_TIERED_EDGE_FLOOR",
        "PGG2_V47F_LARGE_SIZE_DOWNSIZE",
        "PGG2_V47F_SIZE_HOLD_CAP",
        "PGG2_V47F_MIDHOLD_DUMP_ABORT",
    ):
        if os.environ.get(env_name, "0") != "1":
            log(f"V47F-NO-SEND WARNING: {env_name} env not 1")
    ...
    log(
        f"V47F-NO-SEND fee_bps={fee_bps} creator_fee_bps={creator_fee_bps}"
    )
    ...
    # V47F-specific counters.
    v47f_floor_total = 0
    v47f_floor_pass_total = 0
    v47f_floor_block_total = 0
    v47f_floor_blocker_counts: Counter = Counter()
    v47f_floor_pass_by_tier: Counter = Counter()
    v47f_downsize_attempts = 0
    v47f_downsize_success = 0
    v47f_downsize_fail = 0
    v47f_holdcap_applied_by_tier: Counter = Counter()
    v47f_midhold_abort_count = 0
    v47f_midhold_abort_actions: Counter = Counter()
    v47f_midhold_abort_per_mint: Dict[str, Dict[str, Any]] = {}
    ...
    async def _shred_listener():
        ...
        try:
            ...
            log(f"V47F-NO-SEND ws_import_err={exc}")
            return
        url = os.environ.get("SOLANATRACKER_RPC_WS", "")
        if not url:
            log("V47F-NO-SEND no_ws_url")
            return
        backoff = 2.0
        while not shred_stop.is_set():
            try:
                async with websockets.connect(
                    ...
                    log("V47F-NO-SEND shred_subscribed")
                    async for raw in ws:
                ...
                log(f"V47F-NO-SEND shred_reconnect "
    ...
    def _maybe_evaluate(
        ...
        nonlocal v47f_floor_total, v47f_floor_pass_total
        nonlocal v47f_floor_block_total
        nonlocal v47f_downsize_attempts, v47f_downsize_success
        nonlocal v47f_downsize_fail
        ...
        if tb_mode == MODE_BLOCK:
            ...
            log(
                f"PGG2-V47F-REPLACEMENT-SCAN ts={ts_ms_now} "
        ...
        if bg_pass:
            ...
            if d_size is None:
                ...
                if downsized_bool:
                    log(
                        f"PGG2-V47F-REPLACEMENT-SELECTED mint={_short(mint)} "
                        f"size={final_size:.4f} slot={slot} "
                        f"orig_size={original_selected_size:.4f} "
                        f"action=downsized"
                    )

        # --- V47F Phase 2 + Phase 3: size-tiered floor + large-size downsize.
        # Applied AFTER V47D boundary guard + downsize, BEFORE final candidate
        # record build. If V47F floor fails, attempt downsize ladder using
        # already-computed size_results (curve-aware, not linear).
        v47f_floor_total += 1
        cand_exp_pnl = float(final_r1["expected_pnl"])
        v47f_pass, v47f_reason = evaluate_size_edge_floor(
            float(final_size), cand_exp_pnl,
            logger=log, mint_for_log=mint,
        )
        v47f_action = "v47f_floor_pass"
        v47f_orig_for_downsize = float(final_size)
        if not v47f_pass:
            v47f_floor_block_total += 1
            v47f_floor_blocker_counts[v47f_reason or "unknown"] += 1
            # Try V47F downsizer (only fires for size >= 0.030).
            v47f_downsize_attempts += 1

            def _v47f_pnl_at(sz: float) -> float:
                r = size_results.get(float(sz))
                if r is None:
                    return 0.0
                r1_at = r.get("r1")
            ...
            if new_size is None:
                v47f_downsize_fail += 1
                log(
                    f"PGG2-V47F-DOWNSIZE-BLOCK mint={_short(mint)} "
                    f"orig_size={v47f_orig_for_downsize:.4f} "
                    f"floor_reason={v47f_reason} "
                    f"downsize_reason={new_reason} action=block"
                )
                return
            else:
                v47f_downsize_success += 1
                downsized_bool = True
                downsize_action = "v47f_downsized"
                final_size = float(new_size)
                final_res = size_results[final_size]
                final_r1 = final_res["r1"]
                final_g = final_res["guard"]
                v47f_action = "v47f_floor_blocked_downsize_pass"
                log(
                    f"PGG2-V47F-DOWNSIZE-SELECTED mint={_short(mint)} "
                    f"orig_size={v47f_orig_for_downsize:.4f} "
                    f"new_size={final_size:.4f} new_pnl={new_pnl:+.6f} "
                    f"reason={new_reason}"
                )
        else:
            v47f_floor_pass_total += 1
        v47f_floor_pass_by_tier[f"{final_size:.3f}"] += 1

        # Record V47F hold caps (used for analytics; observation loop applies
        # them through the existing MAX_HOLD/EXTEND_IF_POS hooks).
        v47f_caps = get_hold_caps(float(final_size))
        v47f_holdcap_applied_by_tier[f"{final_size:.3f}_hold_{v47f_caps['max_hold_ms']}"] += 1
        ...
        rec = {
            ...
            "v47f_floor_pass": True,
            "v47f_floor_required": float(required_floor_for_size(float(final_size))),
            "v47f_max_hold_ms": int(v47f_caps["max_hold_ms"]),
            "v47f_max_extend_ms": int(v47f_caps["max_extend_ms"]),
            "v47f_extend_allowed_default": bool(v47f_caps["extend_allowed"]),
            "v47f_action": str(v47f_action),
        ...
        jsonl_fp.flush()
        log(
            f"V47F-CANDIDATE mint={_short(mint)} "
            ...
            for mint in list(hot_mint_last_seen.keys())[: args.max_hot_mints]:
                ...
                for p in new_points:
                    ...
                    for key, rec in list(pending_candidates.items()):
                        ...
                        # --- V47F Phase 5: mid-hold dump abort -----
                        # Maintain per-position peak trackers.
                        prev_peak_quote = float(rec.get("v47f_peak_quote", 0.0))
                        prev_peak_vsol = float(rec.get("v47f_peak_vsol", 0.0))
                        prev_peak_pnl = float(rec.get("v47f_peak_pnl", -1e18))
                        prev_last_quote = float(rec.get("v47f_last_quote", 0.0))
                        prev_neg_grad_ct = int(
                            rec.get("v47f_neg_grad_count", 0)
                        )
                        prev_had_above = bool(
                            rec.get("v47f_had_been_above_bank_or_scratch", False)
                        )
                        ...
                        rec["v47f_peak_quote"] = new_peak_quote
                        rec["v47f_peak_vsol"] = new_peak_vsol
                        rec["v47f_peak_pnl"] = new_peak_pnl
                        rec["v47f_last_quote"] = cur_quote
                        rec["v47f_neg_grad_count"] = new_neg_grad_ct
                        rec["v47f_had_been_above_bank_or_scratch"] = new_had_above
                        ...
                        v47f_abort, v47f_act, v47f_abreason = should_abort_midhold(
                            pos_state, {}, logger=log,
                        )
                        if v47f_abort:
                            v47f_midhold_abort_count += 1
                            v47f_midhold_abort_actions[str(v47f_act)] += 1
                            v47f_midhold_abort_per_mint[mint] = {
                                "lag_ms": int(lag),
                                "action": v47f_act,
                                "reason": v47f_abreason,
                                "pnl_at_abort": float(pnl_now),
                            }
                            # Map action -> observed label.
                            if v47f_act == ACTION_SCRATCH:
                                ab_label = "scratch"
                                ab_pnl = max(float(pnl_now), 0.0)
                            elif v47f_act == ACTION_FLAT:
                            ...
                            rec["v47f_midhold_abort_action"] = str(v47f_act)
                            rec["v47f_midhold_abort_reason"] = str(v47f_abreason)
                            jsonl_fp.write(
                                json.dumps({"type": "v47f_observed", **{
                                    k: rec[k] for k in (
                                        "mint", "decision_ts_ms",
                                        "selected_size_sol",
                                        "observed_label_pnl",
                                ...
                                "v47f_midhold_abort_action": str(v47f_act),
                                "v47f_midhold_abort_reason": str(v47f_abreason),
                                "pnl_at_abort": float(pnl_now),
                                }) + "\n"
                            )
                            jsonl_fp.flush()
                            ...
                            log(
                                f"V47F-OBSERVED midhold_abort={v47f_act} "
                                f"label={ab_label} pnl={ab_pnl:+.6f} "
                                f"lag={lag}ms mint={_short(mint)} "
                                f"reason={v47f_abreason}"
                            )
                            continue
                        ...
                        if pnl_now >= BANK_TH:
                            ...
                            pending_candidates.pop(key, None)
                            log(
                                f"V47F-OBSERVED bank pnl={pnl_now:+.6f} "
                                f"lag={lag}ms mint={_short(mint)} "
                                f"size={size_sol:.4f}"
                            )
                            continue
                        if pnl_now <= LOSS_TH:
                            ...
                            pending_candidates.pop(key, None)
                            log(
                                f"V47F-OBSERVED clamp_loss pnl={pnl_now:+.6f} "
                                f"lag={lag}ms mint={_short(mint)} "
                                f"size={size_sol:.4f}"
                            )
                            continue
                        v47f_caps_runtime = get_hold_caps(float(size_sol))
                        rt_max_hold = int(v47f_caps_runtime["max_hold_ms"])
                        rt_max_extend = int(v47f_caps_runtime["max_extend_ms"])
                        rt_extend_default = bool(
                            v47f_caps_runtime["extend_allowed"]
                        )
                        hold_ms = rt_max_hold
                        ...
                        if lag >= hold_ms:
                            ...
                            pending_candidates.pop(key, None)
                            log(
                                f"V47F-OBSERVED {label_kind} pnl={pnl_now:+.6f} "
                                f"lag={lag}ms mint={_short(mint)}"
                            )

            if now_ts >= next_progress_ms:
                log(
                    f"V47F-NO-SEND progress elapsed_s="
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# V47F - Size-Tiered Floor + Mid-Hold Dump Abort No-Send Report\n\n")
        ...
        f.write("\n## V47F size-edge floor stats\n\n")
        f.write(f"- evaluations: {v47f_floor_total}\n")
        f.write(f"- pass: {v47f_floor_pass_total}\n")
        f.write(f"- block: {v47f_floor_block_total}\n")
        f.write("- pass distribution by final size:\n")
        if v47f_floor_pass_by_tier:
            for k in sorted(v47f_floor_pass_by_tier.keys()):
                f.write(f"  - size={k}: {v47f_floor_pass_by_tier[k]}\n")
        else:
            f.write("  - (none)\n")
        f.write("- blocker reasons:\n")
        if v47f_floor_blocker_counts:
            for k, v in v47f_floor_blocker_counts.most_common(20):
                f.write(f"  - {k}: {v}\n")
        else:
            f.write("  - (none)\n")

        f.write("\n## V47F downsize stats\n\n")
        f.write(f"- v47f_downsize_attempts: {v47f_downsize_attempts}\n")
        f.write(f"- v47f_downsize_success: {v47f_downsize_success}\n")
        f.write(f"- v47f_downsize_fail: {v47f_downsize_fail}\n")

        f.write("\n## V47F hold caps applied\n\n")
        f.write("- by selected size:\n")
        if v47f_holdcap_applied_by_tier:
            for k in sorted(v47f_holdcap_applied_by_tier.keys()):
                f.write(f"  - {k}: {v47f_holdcap_applied_by_tier[k]}\n")
        else:
            f.write("  - (none)\n")

        f.write("\n## V47F mid-hold dump abort stats\n\n")
        f.write(f"- abort_count: {v47f_midhold_abort_count}\n")
        f.write("- by action:\n")
        if v47f_midhold_abort_actions:
            for k, v in v47f_midhold_abort_actions.most_common(10):
                f.write(f"  - {k}: {v}\n")
        else:
            f.write("  - (none)\n")
        if v47f_midhold_abort_per_mint:
            f.write("- per-mint detail:\n")
            for mk, dv in list(v47f_midhold_abort_per_mint.items())[:30]:
                f.write(
        ...
        # V47F-specific verdict assertions.
        ...
        if len(candidates) == 0:
            f.write(
                f"- V47F yielded 0 candidates in {elapsed_s:.0f}s. "
                f"two_buyer_total={two_buyer_total}, "
                f"two_buyer_block={two_buyer_block}, "
                f"boundary_guard_total={boundary_guard_total}, "
                f"v47f_floor_total={v47f_floor_total}, "
                f"v47f_floor_block_total={v47f_floor_block_total}, "
                f"v47f_downsize_success={v47f_downsize_success}, "
                f"v47f_downsize_fail={v47f_downsize_fail}, "
                f"replacement_scans={replacement_scan_count}. "
                f"Top mb-gate blockers: "
                f"{dict(mb_gate_blocker_counts.most_common(5))}. "
                f"Top V47F floor blockers: "
                f"{dict(v47f_floor_blocker_counts.most_common(5))}.\n"
            )
        elif clamp_count + expired_loss_count > 0:
            f.write(
                f"- V47F produced {len(candidates)} candidate(s) but "
                f"{clamp_count + expired_loss_count} resulted in observed "
                f"negative outcomes. The V47F floor + dump abort did not "
                f"catch every failure pattern.\n"
            )
        elif clean_close_pass:
            f.write(
                f"- V47F PASSED clean-close: {len(candidates)} candidates, "
            ...
            f.write(
                f"- V47F produced {len(candidates)} candidate(s) with "
            ...
            f.write(
                f"- V47F produced {len(candidates)} candidate(s) without "
                f"negatives but did not reach target ({args.target_pass}). "
                f"V47F floor blocks: {v47f_floor_block_total}; "
                f"V47F downsize success/fail: "
                f"{v47f_downsize_success}/{v47f_downsize_fail}.\n"
            )

    log(f"V47F-NO-SEND wrote {md_path}")
    log(
        f"V47F-NO-SEND done elapsed_s={elapsed_s:.1f} "
        f"candidates={len(candidates)} "
