"""V47I - No-Send capture with V47I 500-1000ms MEDIUM-RUG VETO added
on top of V47H (UNCHANGED).

Forks pgg2_v47h_no_send_capture.py and adds:
  - V47I medium-window buffer (wraps V47H sell-aware buffer; adds quote
    history tracking and aged curve deltas).
  - V47I medium-rug veto called AFTER V47H rug veto (i.e., the LAST
    pre-entry filter in the chain).
  - V47I veto telemetry + shadow-only collection for blocked entries.

V47H base architecture is UNCHANGED. All V47B/C/D/E/F/G/H modules invoked
identically. The medium-rug veto adds one more causal block after V47H;
candidates that pass V47H but fail V47I are recorded as shadow-only
(do not count toward target_pass).
...
for _pat in _FORBIDDEN_CALL_PATTERNS:
    if _re.search(_pat, _src):
        sys.stderr.write(
            f"V47I-NO-SEND-ABORT forbidden_call_pattern={_pat}\n"
        )
        sys.exit(2)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--out-jsonl",
        default="/root/piggy/data/v47i_no_send_decisions.jsonl",
    )
...
async def amain() -> int:
    ...
    try:
        ...
        print(f"V47I-NO-SEND-ABORT import:{type(exc).__name__}:{exc}")
        return 2
    ...
    log(
        f"V47I-NO-SEND start sizes={SIZE_SWEEP_SOL} "
    ...
    for env_name in (
        ...
        if os.environ.get(env_name, "0") != "1":
            log(f"V47I-NO-SEND WARNING: {env_name} env not 1")
    ...
    log(
        f"V47H-NO-SEND fee_bps={fee_bps} creator_fee_bps={creator_fee_bps}"
    )
    ...
    # V47H wraps V47C — adds sell breadth + curve-delta history.
    ...
    # Invariant tracking: counts must remain identical to V47G baseline.
    ...
    async def _shred_listener():
        ...
        try:
            ...
            log(f"V47H-NO-SEND ws_import_err={exc}")
            return
        url = os.environ.get("SOLANATRACKER_RPC_WS", "")
        if not url:
            log("V47H-NO-SEND no_ws_url")
            return
        backoff = 2.0
        while not shred_stop.is_set():
            try:
                async with websockets.connect(
                    ...
                    log("V47H-NO-SEND shred_subscribed")
                    async for raw in ws:
                ...
                log(f"V47H-NO-SEND shred_reconnect "
    ...
    def _maybe_evaluate(
        ...
        # V47H base-invariant: confirm v47g floor + caps invoked once per
        # admitted candidate (matches V47G call counts).
        v47h_base_invariant_calls["v47g_floor"] += 1
        v47h_base_invariant_calls["v47g_get_hold_caps"] += 1

        # --- V47H Phase 2: pre-entry rug veto ---------------------------
        # Pull sell-stats and curve-history at decision time (ALL CAUSAL:
        # buffer methods use only events with ts <= ts_ms_now).
        ...
        if not v47h_pass:
            ...
            jsonl_fp.flush()
            log(
                f"V47H-SHADOW-BLOCK mint={_short(mint)} "
                f"size={final_size:.4f} reasons={'|'.join(v47h_fired)}"
            )
            return
        v47h_veto_pass_total += 1

        # --- V47I Phase 4: medium-window pre-entry veto ---------------
        # Pull buyer 500/1000ms windows, sell 500/1000ms windows, curve
        # delta trajectory with ages, and quote history at decision time.
        # ALL CAUSAL.
        v47i_buyer_stats = {
            "unique_buyers_250ms": int(snap.get("unique_buyers_250ms", 0)),
            "unique_buyers_500ms": int(snap.get("unique_buyers_500ms", 0)),
        ...
        v47i_curve_history = buffer_.curve_history(mint, int(ts_ms_now))
        ...
        v47i_pass, v47i_fired = v47i_evaluate_medium_rug_veto(
            ...
            curve_history=v47i_curve_history,
        ...
        if not v47i_pass:
            ...
            jsonl_fp.write(json.dumps({
                ...
                "curve_delta_500ms_n": int(len(v47i_curve_history.get("vsol_deltas_last_500ms", []))),
                "curve_delta_1000ms_n": int(len(v47i_curve_history.get("vsol_deltas_last_1000ms", []))),
                "quote_history_n": int(len(v47i_quote_history.get("local_quote_history_5", []))),
                "quote_peak_500ms": v47i_quote_history.get("local_quote_peak_500ms"),
                "quote_last": v47i_quote_history.get("local_quote_last"),
            }) + "\n")
            jsonl_fp.flush()
            log(
                f"V47I-SHADOW-BLOCK mint={_short(mint)} "
        ...
        rec = {
            ...
            # V47H veto telemetry on admitted candidates.
            "v47h_veto_pass": True,
            "v47h_vetos_fired": [],
            "v47h_sell_count_250ms": int(
                v47h_sell_stats.get("sell_count_250ms", 0)
            ),
            "v47h_unique_sellers_250ms": int(
                v47h_sell_stats.get("unique_sellers_250ms", 0)
            ),
            "v47h_largest_sell_tokens_250ms": int(
                v47h_sell_stats.get("largest_sell_tokens_250ms", 0)
            ),
            "v47h_curve_deltas_500ms_len": int(
                len(v47h_curve_history.get("vsol_deltas_last_500ms", []))
            ),
            "v47h_curve_deltas_1000ms_len": int(
                len(v47h_curve_history.get("vsol_deltas_last_1000ms", []))
            ),
        }
        if downsized_bool:
        ...
        jsonl_fp.flush()
        log(
            f"V47H-CANDIDATE mint={_short(mint)} "
        ...
        while _now_ms() < deadline_ms:
            ...
            for mint in list(hot_mint_last_seen.keys())[: args.max_hot_mints]:
                ...
                for p in new_points:
                    ...
                    # V47I: record a reference local sell quote per mint
                    # so that the medium-rug veto C (quote weakening) has
                    # at least 5 historical points. The reference position
                    # size is fixed at 0.005 SOL (the smallest sweep size)
                    # so that the quote series is comparable across mints
                    # and across time. This call is causal — uses only the
                    # current curve point. It does NOT alter buy/sell paths.
                    try:
                    ...
                    for key, rec in list(pending_candidates.items()):
                        ...
                        if v47g_drove:
                            ...
                            jsonl_fp.write(
                                json.dumps({"type": "v47h_observed_watchdog",
                                    **{k: rec[k] for k in (
                                        "mint", "decision_ts_ms",
                                        "selected_size_sol",
                                        "observed_label_pnl",
            ...
            if now_ts >= next_progress_ms:
                log(
                    f"V47H-NO-SEND progress elapsed_s="
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# V47H - Pre-Entry Rug Veto No-Send Report\n\n")
        ...
        # --- V47H rug veto section (Phase 6 telemetry) ---
        f.write("\n## V47H rug-veto stats\n\n")
        ...
        f.write("\n## V47H base-invariant check\n\n")
        f.write(
            f"- PGG2-V47H-BASE-INVARIANT-CHECK msg=v47g_modules_unchanged "
            f"calls={dict(v47h_base_invariant_calls)}\n"
        )
        # Specific V47H invariants on admitted candidates.
        no_blowoff_admitted = all(
            (float(r.get("expected_pnl", 0.0))
             / max(1e-12, float(r.get("selected_size_sol", 1.0)))) < 2.0
            for r in candidates
        ) if candidates else True
        no_ub2_tbs_gt_050_admitted = all(
            not (int(r.get("unique_buyers_250ms", 0)) == 2
                 and float(r.get("top_buyer_share_250ms", 0.0)) > 0.50)
            for r in candidates
        ) if candidates else True
        no_sub500_rug_admitted = all(
            not (r.get("observed_label_kind") in ("clamp_loss", "expired_loss")
                 and r.get("observed_label_lag_ms") is not None
                 and int(r.get("observed_label_lag_ms", 0)) < 500)
            for r in candidates
        ) if candidates else True
        f.write(f"- no_blowoff_anomaly_admitted (ratio<2): {no_blowoff_admitted}\n")
        f.write(f"- no_ub2_tbs_gt_050_admitted: {no_ub2_tbs_gt_050_admitted}\n")
        f.write(f"- no_sub500_rug_admitted: {no_sub500_rug_admitted}\n\n")

        # --- V47I medium-rug veto section (Phase 4 telemetry) ---
        ...
        # V47I invariant on admitted candidates: no 500-1000ms rugs.
        no_500_1000_rug_admitted = all(
            not (r.get("observed_label_kind") in ("clamp_loss", "expired_loss")
                 and r.get("observed_label_lag_ms") is not None
                 and 500 <= int(r.get("observed_label_lag_ms", 0)) <= 1000)
            for r in candidates
        ) if candidates else True
        f.write(f"- no_500_1000ms_rug_admitted: {no_500_1000_rug_admitted}\n\n")
        ...
        f.write(f"- v47h_no_blowoff_anomaly_admitted: {no_blowoff_admitted}\n")
        f.write(f"- v47h_no_ub2_tbs_gt_050_admitted: {no_ub2_tbs_gt_050_admitted}\n")
        f.write(f"- v47h_no_sub500_rug_admitted: {no_sub500_rug_admitted}\n")
        v47h_overall = bool(overall and no_blowoff_admitted
                            and no_ub2_tbs_gt_050_admitted
                            and no_sub500_rug_admitted)
        f.write(f"- v47i_no_500_1000ms_rug_admitted: {no_500_1000_rug_admitted}\n")
        v47i_overall = bool(v47h_overall and no_500_1000_rug_admitted)
        f.write(f"- OVERALL_VERDICT: {'PASS' if v47i_overall else 'FAIL'}\n\n")

        f.write("## HONEST ASSESSMENT\n\n")
        if len(candidates) == 0:
            f.write(
                f"- V47H yielded 0 candidates in {elapsed_s:.0f}s. "
                f"v47h_veto_total={v47h_veto_total}, "
                f"v47h_veto_block={v47h_veto_block_total}, "
                f"shadow_only={len(v47h_shadow_candidates)}, "
                ...
                f"Top V47H veto reasons: "
                f"{dict(v47h_veto_reason_counts.most_common(5))}.\n"
            )
        elif clamp_count + expired_loss_count > 0:
            f.write(
                f"- V47H produced {len(candidates)} candidate(s) but "
                f"{clamp_count + expired_loss_count} resulted in observed "
                f"negative outcomes. The V47H rug veto did not catch every "
                f"failure pattern. Residual rugs ARE caught by the V47G "
                f"watchdog mid-hold abort.\n"
            )
        elif clean_close_pass:
            f.write(
                f"- V47H PASSED clean-close: {len(candidates)} candidates, "
                f"closed_nonneg={cc['closed_nonneg']}, closed_neg=0, "
                f"pending=0, open=0. V47G+V47H stack operational. "
                f"V47H blocked {v47h_veto_block_total} shadow candidates.\n"
            )
        elif pending_count > 0:
            f.write(
                f"- V47H produced {len(candidates)} candidate(s) with "
            ...
            f.write(
                f"- V47H produced {len(candidates)} candidate(s) without "
                f"negatives but did not reach target ({args.target_pass}). "
                f"V47H veto blocks: {v47h_veto_block_total}; "
                f"V47G floor blocks: {v47g_floor_block_total}.\n"
            )

    log(f"V47H-NO-SEND wrote {md_path}")
    log(
        f"V47H-NO-SEND done elapsed_s={elapsed_s:.1f} "
        f"candidates={len(candidates)} "
    ...
    overall_pass = bool(
        ...
        and no_size_geq_020_exp_pnl_lt_002
        and all_holds_within_caps
        and no_blowoff_admitted
        and no_ub2_tbs_gt_050_admitted
        and no_sub500_rug_admitted
        and no_500_1000_rug_admitted
    )
