"""V47C - No-Send capture with guarded-branch size sweep
+ multi-buyer flow quality gate + size-by-breadth caps.

Forks pgg2_v47b_no_send_capture.py and adds:
  - signer-aware buyer stats (per-window unique signers + top buyer share)
  - multi-buyer gate (Phase 2) BEFORE size sweep
  - size cap (Phase 3) AFTER guarded-branch selection
  - persists full V47C feature set in JSONL for downstream replay

Order of checks per candidate:
  1. Compute buyer_stats from signer-aware buffer.
  2. Multi-buyer gate. If fail -> block, log reason.
  3. Run V47B guarded-branch sweep over all sizes.
  4. Among selectable sizes, apply Phase 3 size cap.
  5. Select smallest capped size with expected_pnl >= required_profit.
  6. If none survive -> block.
...
for _pat in _FORBIDDEN_CALL_PATTERNS:
    if _re.search(_pat, _src):
        sys.stderr.write(
            f"V47C-NO-SEND-ABORT forbidden_call_pattern={_pat}\n"
        )
        sys.exit(2)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--out-jsonl",
        default="/root/piggy/data/v47c_no_send_decisions.jsonl",
    )
...
async def amain() -> int:
    ...
    try:
        ...
        print(f"V47C-NO-SEND-ABORT import:{type(exc).__name__}:{exc}")
        return 2
    ...
    log(
        f"V47C-NO-SEND start sizes={SIZE_SWEEP_SOL} "
    ...
    if os.environ.get("PGG2_V40_DISABLE_PUMPBC_SAME_ROUTE", "0") != "1":
        log("V47C-NO-SEND WARNING: PGG2_V40_DISABLE_PUMPBC_SAME_ROUTE != 1")
    if os.environ.get("V47C_NO_SEND", "0") != "1":
        log("V47C-NO-SEND WARNING: V47C_NO_SEND env not 1")
    if os.environ.get("PGG2_V47C_MULTI_BUYER_GATE", "0") != "1":
        log("V47C-NO-SEND WARNING: PGG2_V47C_MULTI_BUYER_GATE env not 1")
    if os.environ.get("PGG2_V47C_SIZE_BY_BREADTH", "0") != "1":
        log("V47C-NO-SEND WARNING: PGG2_V47C_SIZE_BY_BREADTH env not 1")
    ...
    log(
        f"V47C-NO-SEND fee_bps={fee_bps} creator_fee_bps={creator_fee_bps}"
    )
    ...
    async def _shred_listener():
        ...
        try:
            ...
            log(f"V47C-NO-SEND ws_import_err={exc}")
            return
        url = os.environ.get("SOLANATRACKER_RPC_WS", "")
        if not url:
            log("V47C-NO-SEND no_ws_url")
            return
        backoff = 2.0
        while not shred_stop.is_set():
            try:
                async with websockets.connect(
                    ...
                    log("V47C-NO-SEND shred_subscribed")
                    async for raw in ws:
                ...
                log(f"V47C-NO-SEND shred_reconnect "
    ...
    def _maybe_evaluate(
        ...
        # Signer-aware buyer stats (per-window unique signers + top share).
        ...
        # --- V47C Phase 2: Multi-buyer flow quality gate (BEFORE size sweep) ---
        mb_gate_total += 1
        ...
        # --- V47C Phase 3: Apply size-by-breadth cap to each selectable size ---
        # Build the buyer_stats for size-cap (uses ub_250, ub_500, tbs, pbs).
        buyer_stats_for_cap = {
            "unique_buyers_250ms": int(snap.get("unique_buyers_250ms", 0)),
        ...
        # For each selectable size, evaluate the size-cap. Keep only those
        # for which the cap returns a capped_size equal to the requested
        # size (we don't shrink one size into another; we just admit/block).
        ...
        # Admissible sizes are those where capped == s (i.e., the size is
        # honestly admissible at this breadth). We do NOT allow the cap to
        # silently shrink: if you asked for 0.05 but cap says 0.020, we
        # treat 0.05 as BLOCKED at this signal (and 0.020 is its own
        # independent candidate size — already evaluated in size_results
        # and either selectable or not).
        admissible_sizes = [
        ...
        # Pick smallest admissible size with expected_pnl >= required_profit.
        ...
        rec = {
            ...
            "size_cap_applied": bool(
                ...
                and abs(cap_capped - float(selected_size)) > 1e-9
            ),
                ...
                for p in new_points:
                    ...
                    for key, rec in list(pending_candidates.items()):
                        ...
                        if pnl_now >= BANK_TH:
                            ...
                            log(
                                f"V47C-OBSERVED bank pnl={pnl_now:+.6f} "
                        ...
                        if pnl_now <= LOSS_TH:
                            ...
                            pending_candidates.pop(key, None)
                            log(
                                f"V47C-OBSERVED clamp_loss pnl={pnl_now:+.6f} "
                                f"lag={lag}ms mint={_short(mint)} "
                                f"size={size_sol:.4f}"
                            )
                            continue
                        ...
                        if lag >= hold_ms:
                            ...
                            pending_candidates.pop(key, None)
                            log(
                                f"V47C-OBSERVED {label_kind} pnl={pnl_now:+.6f} "
                                f"lag={lag}ms mint={_short(mint)}"
                            )

            if now_ts >= next_progress_ms:
                log(
                    f"V47C-NO-SEND progress elapsed_s="
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# V47C - Multi-Buyer Gated Guarded-Branch No-Send Report\n\n")
        ...
        f.write("- rules_path: (in-code constants; see "
                "pgg2_v47c_multi_buyer_gate.py + pgg2_v47c_size_cap.py)\n\n")
        ...
        f.write("\n## V47C size-cap stats\n\n")
        f.write(f"- size_cap_applied_to_selected: {size_cap_applied_count}\n")
        f.write(f"- size_cap_block_reasons:\n")
        if size_cap_block_counts:
            for k, v in size_cap_block_counts.most_common(10):
                f.write(f"  - {k}: {v}\n")
        else:
            f.write("  - (none)\n")

        f.write("\n## V47C candidate entries\n\n")
        ...
        if not selected_size_dist:
            f.write("  - (none)\n")
        # Distribution of unique_buyers_250ms across selected.
        ub_dist: Counter = Counter()
        for r in candidates:
            ub_dist[int(r.get("unique_buyers_250ms", 0))] += 1
        f.write(f"- unique_buyers_250ms distribution (selected):\n")
        if ub_dist:
        ...
        all_size_cap_respected = (
            all(
                (
                    int(r.get("unique_buyers_250ms", 0)) >= 3
                    if float(r.get("selected_size_sol", 0.0)) > 0.020
                    else int(r.get("unique_buyers_250ms", 0)) >= 2
                )
                for r in candidates
            ) if candidates else False
        )
        ...
        f.write(f"- all_candidates_multi_buyer(>=2): {all_multi_buyer}\n")
        f.write(f"- all_candidates_size_cap_respected: {all_size_cap_respected}\n")
        ...
        if candidates:
            f.write(
                "| # | mint | size | ub_250 | tbs | sz_cap_applied | "
                "exp_pnl | adv_pnl | adv_branch | obs_kind | obs_pnl | obs_lag |\n"
            )
            f.write(
                "|---|------|------|--------|-----|----------------|"
                "---------|---------|------------|----------|---------|---------|\n"
            ...
            for i, r in enumerate(candidates, 1):
                f.write(
                    ...
                    f"{int(bool(r.get('size_cap_applied', False)))} | "
        ...
        f.write("\n## HONEST ASSESSMENT\n\n")
        if len(candidates) == 0:
            if sim_evals_total == 0 and mb_gate_total == 0:
                f.write(
                    "- V47C yielded 0 candidates AND 0 sim evals (no shred "
            ...
            elif mb_gate_pass_total == 0 and mb_gate_total > 0:
                top_mb_blockers = dict(mb_gate_blocker_counts.most_common(5))
                f.write(
                    f"- V47C yielded 0 candidates because the multi-buyer "
                    f"gate blocked all {mb_gate_total} pending-flow events. "
                    f"Top mb-gate blockers: {top_mb_blockers}. "
                    f"This means the live window had no multi-signer pre-curve "
                    f"buy clusters meeting our quality bar (>=2 unique signers, "
                    f">=2 pending count, buy>sell SOL, top_share<=0.75).\n"
                )
            elif sim_evals_total == 0:
                f.write(
                    f"- V47C yielded 0 candidates: {mb_gate_pass_total} "
                    f"mb-gate passes but 0 reached sim eval.\n"
                )
            else:
                top_blockers = dict(blocker_counts.most_common(5))
                top_cap_blockers = dict(size_cap_block_counts.most_common(5))
                f.write(
                    f"- V47C yielded 0 candidates from {sim_evals_total} "
                    f"sim evals after {mb_gate_pass_total}/{mb_gate_total} "
                    f"mb-gate passes. Top branch/guard blockers: {top_blockers}. "
                    f"Top size-cap blockers: {top_cap_blockers}.\n"
                )
        elif clamp_count + expired_loss_count > 0:
            f.write(
                f"- V47C produced {len(candidates)} candidate(s) but "
                f"{clamp_count + expired_loss_count} resulted in losses. "
                "This indicates the multi-buyer gate + size cap was "
                "insufficient on this fresh window.\n"
            )
        elif meets_target and zero_neg:
            f.write(
                f"- V47C PASSED: {len(candidates)} candidates, 0 negative "
                f"outcomes, all adverse branches in WIN/SAFE_FAIL, all "
                f"unique_buyers_250ms >= 2, size caps respected. "
                "Eligible for dry-live.\n"
            )
        else:
            f.write(
                f"- V47C produced {len(candidates)} candidate(s) without "
                f"negatives but did not reach target ({args.target_pass}).\n"
            )

    log(f"V47C-NO-SEND wrote {md_path}")
    log(
        f"V47C-NO-SEND done elapsed_s={elapsed_s:.1f} "
        f"candidates={len(candidates)} "
